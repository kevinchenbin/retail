...
edtReceivedMoney->Text =
 FormatFloat("#,##0.00",RecCash +Youhuiquan+PostCard+ negTotalDiscountedPrice +quhuiyuan->FieldByName("voucher")->AsFloat+aq->FieldByName("recemondy")->AsFloat);// qusale->FieldByName("Checked")->AsFloat);
...
void TfrmCheckQuery::Query()
{
   ...
   //	dsetCheck->Connection = con1;
	m_sql = "select sys_user.name as username,TotalAmount ,HdTime, OperatorID, TotalDiscountedPrice, TotalFixedPrice,RecCardMoney,RecCash,poscard,youhuiquan  from BS_RetailNoteHeader left join sys_user on BS_RetailNoteHeader.OperatorID=sys_user.id where";  // where
