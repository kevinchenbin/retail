...

{
	if (Key == '\r') {
        ...
        if (dsetNtHeader->FieldByName("CardSN")->AsAnsiString != edcard->Text) {
			AnsiString sql;
			sql = "select ID,PassWord,Available,datediff(d,getdate(),youxiaodate) as t from CUST_MemberInfo where CUST_MemberInfo.stgidex = "+IntToStr(fstgid)+" and cardsn = '" + edcard->Text + "'" ;
			aquery->Close();
			aquery->SQL->Clear();
			aquery->SQL->Add(sql);
			aquery->Open();
			if (aquery->RecordCount > 0) {
				if (!aquery->FieldByName("Available")->AsBoolean ) {
					ShowMsg(this->Handle,"会员卡已无效！",3);
					return;
				}
				if (aquery->FieldByName("t")->AsInteger < 0 ) {
					sql = "update CUST_MemberInfo set Available = 0 where ID = " + aquery->FieldByName("ID")->AsAnsiString ;
					aquery->Close();
					aquery->SQL->Clear();
					aquery->SQL->Add(sql);
					aquery->ExecSQL();
				...
				if (aquery->FieldByName("PassWord")->AsAnsiString != "" ) {
					TfrmPwd *frm = new TfrmPwd(Application);
					if (frm->ShowModal() == mrOk ) {
						if (frm->GetPassword()!= aquery->FieldByName("PassWord")->AsAnsiString) {
				...
				dsetNtHeader->Edit();
				dsetNtHeader->FieldByName("MemberID")->AsInteger = aquery->FieldByName("ID")->AsInteger ;
