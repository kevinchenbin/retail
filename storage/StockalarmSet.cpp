...
void TfrmalarmSet::search()
{
	...
	sql = "SELECT rank() over(order by STK_Bookinfo.ID) as xuhao,STK_Bookinfo.ID,BS_BookCatalog.Name,BS_BookCatalog.Price, BS_BookCatalog.ISBN,"
		...
		" STK_BookInfo.Amount,STK_Bookinfobk.maxnum,STK_Bookinfobk.minnum, "
		...
		" left join stk_bookinfobk on stk_bookinfo.id = stk_bookinfobk.bkinfoid"
	...
	try {
		aquerydisplay->Open();
                ...
                ShowMsg(Handle ,"请输入正确的检索条件！",3);
		return;
	}
	if (aquerydisplay->RecordCount == 0) {

                ShowMsg(Handle ,"没有查询记录！",3);
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmalarmSet::dgalarmlistCellClick(TColumn *Column)
{
	...
	edtmax->Text = aquerydisplay->FieldByName("maxnum")->AsAnsiString ;
	edtmin->Text = aquerydisplay->FieldByName("minnum")->AsAnsiString ;
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmalarmSet::RzButton2Click(TObject *Sender)
{
	...
	strwhere = "(";
	if (cball->Checked ) {
		strwhere = strwhere + " select STK_BookInfo.id "
		" FROM STK_BookInfo LEFT JOIN "
		" BS_BookCatalog ON STK_BookInfo.BkcatalogID = BS_BookCatalog.ID"
		" LEFT OUTER JOIN CUST_CustomerInfo ON STK_BookInfo.SupplierID = CUST_CustomerInfo.ID"
		" LEFT OUTER JOIN BS_BookType ON BS_BookCatalog.smallBookTypeID = BS_BookType.ID"
	...
	ShowMsg(this->Handle,"库存预警成功设置！",3);
...
void __fastcall TfrmalarmSet::edsupplierKeyPress(TObject *Sender, wchar_t &Key)
{
	if (Key == '\r') {
		AnsiString sql;
		//供应商
		sql = "select ID,Name from CUST_CustomerInfo where type = 1 and customerstate=1 and stgid="+IntToStr(fstgid)+" and Name like '%" + edsupplier->Text + "%'";
		aquery->Close();
		aquery->SQL->Clear();
		aquery->SQL->Add(sql);
		aquery->Open();
