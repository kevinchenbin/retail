...
void __fastcall TfrmStockAdjust::BtnOKClick(TObject *Sender)
{
  ...
  if (aqdetail->IsEmpty() ) {
   ...
   if (ShowMsg(Handle ,"确定要修该库存数量吗?",1)==1) {
...
{
	TfrmQryCode* frm = new TfrmQryCode(Application,fcon,Stgid,0,1);
	...
	if (frm->ShowModal() == mrOk) {
		...
		ids =  aq->FieldByName("id")->AsAnsiString;

		sql = "select * from STK_AdjustHeader " + frm->sqlwhere;
		aqhead->Close();
		aqhead->SQL->Clear();
		aqhead->SQL->Add(sql);
		aqhead->Open();
...
//---------------------------------------------------------------------------
void TfrmStockAdjust::ChangeCatalogSearchMode()
{
