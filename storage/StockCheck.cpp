   ...
   try
   {
		...
		if (aquery->RecordCount == 0) {
			...
			if (aquery->IsEmpty() ) {
                                ...
                                ShowMsg(Handle ,"没有该书记录！",3);
			}
			else
			{
				if(ShowMsg(Handle ,"当前盘点库位无该书，是否盘入该书！",1)== 1)
				{
...
void __fastcall Tfrmcheckstock::FormCreate(TObject *Sender)
{
	...
	if (aquery->RecordCount >0) {
        ;
		if (ShowMsg(Handle ,"还有未结束的盘点单，是否继续盘点？确定继续盘点，取消产生新的盘点单。",1)== 1) {
			if (aquery->RecordCount > 1) {
...
void __fastcall Tfrmcheckstock::BtnHorizontalTileClick(TObject *Sender)
{
	...
	localid = 0;//////////
	sql = "select rank() over(order by stk_bookinfo.id) as xuhao,BS_BookCatalog.ID,STK_BookInfo.SupplierID,BS_BookCatalog.Name as bookname,BS_BookCatalog.Barcode,"
			" BS_BookCatalog.UserDefCode,BS_BookCatalog.ISBN,CUST_CustomerInfo.Name as SupplierName,STK_BookInfo.ID as STK_BookinfoID, "
	...
	{
		AnsiString sql;
		sql = "insert into STK_StockInventories(bkcatalogid,BarCode,suppliername,ISBN,bookname, "
			" AbbreviatedName,actualamount,supplierid,STK_bookInfoID,NoteCode,shouldamount,Balance) "
			" select BS_BookCatalog.id,BS_BookCatalog.barcode,CUST_CustomerInfo.name,BS_BookCatalog.isbn,BS_BookCatalog.name, "
			" BS_PressInfo.AbbreviatedName,STK_BookInfo.amount,STK_BookInfo.supplierid,STK_BookInfo.id,'" + Code + "',0,0-STK_BookInfo.amount "
...
{
	if (editnum->Text == "") {
		ShowMsg(Handle ,"还有盘点数据没有修改库存，请修改库存后结束盘点！",0);
		return;
	}
	int value;
...
bool Tfrmcheckstock::DbgridToExcel(TDBGrid* dbg)
{
	...
	temptext = "\n";
	temptext = temptext + "盘点店,'" + labtitle->Caption + ",库位," + cblocal->Text + ",备注," + membk->Text ;
	iFileLength   =   FileSeek(iFileHandle,0,2);
	FileWrite(iFileHandle,temptext.c_str() ,temptext.Length());
...
{
	if (Key == VK_F1 && Btninit->Enabled && Btninit->Visible ) {     //盘点初始化
		Btninit->Click();
	}
	if (Key == VK_F2 && BtnProperties->Enabled && BtnProperties->Visible) {     //开始盘点
		BtnProperties->Click();
	}
	if (Key == VK_F3 && BtnViewList->Enabled && BtnViewList->Visible) {     //盘点修改库存
		BtnViewList->Click();
	}
	if (Key == VK_F4 && BtnViewIcons->Enabled && BtnViewIcons->Visible) {     //结束盘点
		BtnViewIcons->Click();
	}
	if (Key == VK_F5 && BtnHorizontalTile->Enabled && BtnHorizontalTile->Visible) {     //未盘书
		BtnHorizontalTile->Click();
	}
	if (Key == VK_F6 && BtnChangeOptions->Enabled && BtnChangeOptions->Visible) {     //结束本次盘点
		BtnChangeOptions->Click();
	}
	if (Key == VK_F8 && BtnExport->Enabled && BtnExport->Visible) {     //导出
		BtnExport->Click();
	}
	if (Key == VK_F11 && BtnClear->Enabled && BtnClear->Visible) {     //预览
		BtnClear->Click();
	}
	if (Key == VK_F10 && BtnPrint->Enabled && BtnPrint->Visible) {     //打印
		BtnPrint->Click();
	}
...
void __fastcall Tfrmcheckstock::BtninitClick(TObject *Sender)
{
	if (count > 0) {
                 ...
                 ShowMsg(Handle ,"盘点已经初始化，若要重新初始化盘点请先结束本次盘点！",3);
		return;
	}
	TADOQuery *aq = new TADOQuery(this);
	aq->Connection = fcon;
...
void __fastcall Tfrmcheckstock::BtnChangeOptionsClick(TObject *Sender)
{
	...
	aquery->Open();
	if (aquery->RecordCount > 0) {
		ShowMsg(Handle ,"还有盘点单未结束，请先结束盘点单！",3);
		return;
	}
	if (ShowMsg(Handle ,"确定要结束本次盘点？",1)!= 1) {
		return;
	}

	if (!weipanshu) {
		if (ShowMsg(Handle ,"是否将未盘书清零？",1)== 1) {
			BtnHorizontalTile->Click();
			weipanshu = true;
		}
		else
...
//导入
      ...
      String str = " Provider=Microsoft.Jet.OLEDB.4.0;Extended Properties=Excel 8.0;Persist Security Info=false;Data Source=";
        ...
        while(!aqinput->Eof)
        {
          ...
          // aqinsertkuchen->Edit();
          if (aqinput->FieldByName("内码")->AsAnsiString!="") {


           aqinsertkuchen->Append();

           aqinsertkuchen->FieldByName("stk_bookinfoid")->AsAnsiString= aqinput->FieldByName("内码")->AsAnsiString;
           aqinsertkuchen->FieldByName("ShouldAmount")->AsAnsiString  = aqinput->FieldByName("盘点数")->AsAnsiString;
        ...
        aqinput->Close();
        ShowMsg(this->Handle,"导入完成！",3);
        xlsconnect->Connected = false;
        }
        catch(...)
        {

            ShowMsg(this->Handle,"导入失败,请退出重试！",3);
            xlsconnect->Connected = false;
